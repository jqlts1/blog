...
class BaseMixin(object):
    def get_context_data(self, *args, **kwargs):
        ...
        try:
            ...
            context['author'] = u'张特'       #网站作者
            context['website_keywords'] = [u'张特',u'网络营销实战'] #网站关键词
            context['website_desc'] = '分享/交流,网络营销实战/python编程/电子商务/seo/等各种经验' #网站关键词
            ...
            #权重高的3个分类
            # context['fenlei']= Category.objects.order_by('-rank')[:3]
...
class ArticleView(BaseMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['xg_article'] = Category.objects.get(pk=gx_article).article_set.all()[:10]
...
class CategoryView(BaseMixin, ListView):
    '''分类视图'''
    template_name = 'blog/fenlei.html'
    context_object_name = 'article_list'
    paginate_by = settings.PAGE_NUM
    ...
    def get_queryset(self):
        ...
        try:
            ...
            self.category_title = article_list[0].category.name
            self.category_desc = article_list[0].category.info
...
#
# def test(request,pk,url):
#     try:
#         wenzhang = Article.objects.get(pk=pk)   #获取相关的文章
#         fenlei = Category.objects.get(url=url)
#     except Article.DoesNotExist:
#         raise Http404
#     except Category.DoesNotExist:
#         raise Http404
#     return render(request, 'test.html', {'wenzhang':wenzhang,'url':url})